...
int main(int argc, char** argv)
{
  ...
  while(ros::ok()){
    ...
    if (stage_number == stage_1){
      if (pow((x_map - goal.target_pose.pose.position.x), 2) + pow((y_map - goal.target_pose.pose.position.y), 2) < pow (0.6, 2) ){
        cancelGoal();
        stage_number = stage_2;
      }
    }
	      ...
	      case MOVING:
	        ...
	        distance = sqrt(pow(goal.target_pose.pose.position.x - x_map, 2)+ pow(goal.target_pose.pose.position.y - y_map, 2));
              ...
              end_pub_.publish(std_msgs::String(str));
		        ...
		        if (side_move){
              if (goal.target_pose.pose.position.x - x_map > 0)
	              cmd.linear.x = 0.4 * (goal.target_pose.pose.position.x - x_map);
	            else
	              cmd.linear.x = 0.4 * (goal.target_pose.pose.position.x - x_map);
	            if (goal.target_pose.pose.position.y - y_map > 0)
	              cmd.linear.y = 0.4 * (goal.target_pose.pose.position.y - y_map);
	            else
	              cmd.linear.y = 0.4 * (goal.target_pose.pose.position.y - y_map);
		        }
		        else{
      ...
      if (distance > 0.4){
        ...
        cmd.linear.x = -0.1 - 0.04 * distance;
      }
      vel_pub_.publish(cmd);
    }
  };
...
void sendMap(){
  ...
  mapfname = "/home/jfasola/fuerte_workspace/sandbox/map_server_dummy2/map_2_slam.pgm";
  std::string frame_id;
